...
<head>
    ...
    <title>Book Requested / Issues</title>
    <style>
        body {
...
<table>
    ...
    <tr>
        ...
        <%  if ("false".equals(rs.getString("status"))) { %>
                <button type="button" class="request-button">You Have Requested .. !</button>
            <% }else if ("true".equals(rs.getString("status"))) { %>
             <form action="processReturn.jsp" method="post">
	               <input type="hidden" name="bookId" value="<%= rs.getString("request_id") %>">
	                <button type="submit" class="request-button">Return</button>
	            </form>
            <%} %>
        </td>
    </tr>
